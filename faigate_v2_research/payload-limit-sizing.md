...
## 8. Premise update — catalog state as of 2026-09-12

This section updates the "Honest gaps" concern raised in §7: the analysis
targeted **262,144 tokens** because the bundled catalog advertised that value
...
| Model | `max_input_tokens` | MiB needed (EN prose, 6.59 B/tok) |
|-------|-------------------:|-----------------------------------:|
| gpt-5.4 | 1,050,000 | ~6.60 MiB |
| gemini-3.6-flash | 1,048,576 | ~6.59 MiB |
| gemini-3.7-flash | 1,048,576 | ~6.59 MiB |
| gemini-3.8-flash | 1,048,576 | ~6.59 MiB |
| gemini-2.5-pro | 1,048,576 | ~6.59 MiB |
| gemini-2.5-flash | 1,048,576 | ~6.59 MiB |
| gemini-2.5-flash-lite | 1,048,576 | ~6.59 MiB |
| claude-opus-4-6 | 1,000,000 | ~6.30 MiB |
| claude-sonnet-4-6 | 1,000,000 | ~6.30 MiB |
| deepseek-v4-flash-vision-exp | 1,000,000 | ~6.30 MiB |

**What the 2 MiB raise actually achieves today:**

The original recommendation (§5) was to raise to 2 MiB to make the *then-universal* 262,144
token cap reachable for every measured corpus, including worst-case EN prose. That goal is
fully met: 2 MiB ÷ 6.59 B/tok ≈ **318,232 tokens**, which clears 262,144 with headroom.

However, 10 of the 16 belegt caps now sit at 922,000–1,050,000 tokens. Reaching 1,000,000
tokens in EN prose requires ~6.30 MiB of wire body; reaching 1,048,576 tokens requires
~6.59 MiB. Neither is achievable under a 2 MiB limit.

**This is a product decision, not a mis-sized default.** The 2 MiB raise is the
evidence-grounded correction the analysis recommended. Going to ~6.6 MiB to cover the
large-context caps introduces a different trade-off: `_read_json_body` reads the entire body
into memory before any limit comparison, so the peak per-request allocation scales directly
with the cap. At 6.6 MiB, a single concurrent oversized request binds ~6.6 MiB of heap;
with N concurrent requests that multiplies linearly. Whether that trade-off is acceptable
depends on the deployment context (localhost-only vs networked, available RAM, expected
concurrency) and is outside the scope of this sizing analysis. See the project backlog for the
